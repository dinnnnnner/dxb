...
`generate_synthetic_road_scenarios.py` 从真实爆胎发生前的正常四轮轮速中截取基线，注入 9 类可复现扰动，用来检查爆胎报警算法对 hard negative、路面切换和渐变故障的鲁棒性。
...
| 粗糙路面进入冰面 `rough_to_ice` | 先注入四轮相关与轮端独立的粗糙路面振动，再平滑切换到低附着路面；样本循环覆盖驱动空转/TCS、制动滑移/ABS、转弯侧滑/ESC和无明显滑移的匀速滑行 |
| 传感器异常 `sensor_anomaly` | 循环生成尖峰、掉零、卡死、阶跃偏置和噪声突发等变体 |

轮位沿用当前工程假设：`wheel0=FL`、`wheel1=FR`、`wheel2=RL`、`wheel3=RR`。

`rough_to_ice` 的 `scenario_start_s` 是粗糙路段开始时刻，真正进入冰面的时刻保存在 `parameters_json.ice_transition_start_s`。冰面附着系数是用于控制合成强度的 `mu` 代理值，不是由四轮轮速反演得到的真实附着系数。匀速滑行变体不会凭空加入滑移，因为仅靠四轮轮速通常无法直接观察无激励条件下的附着系数突变。
